...
export const LogoLoop = React.memo<LogoLoopProps>(
    ({
        ...
        useResizeObserver(updateDimensions, [containerRef, seqRef], [updateDimensions, logos, gap, logoHeight, isVertical]);

        useImageLoader(seqRef, updateDimensions, [updateDimensions, logos, gap, logoHeight, isVertical]);
        ...
        const renderLogoItem = useCallback(
            (item: LogoItem, key: React.Key) => {
                ...
                const nodeItem = item as { node: React.ReactNode; href?: string; title?: string; ariaLabel?: string };
                const imageItem = item as { src: string; alt?: string; href?: string; title?: string; srcSet?: string; sizes?: string; width?: number; height?: number };

                const content = isNodeItem ? (
                    <span
                        className={cn(
                            'inline-flex items-center gap-3 opacity-60 hover:opacity-100 transition-all duration-300',
                    ...
                    >
                        {nodeItem.node}
                        {nodeItem.title && (
                            <span className="font-serif italic text-sm md:text-base font-medium tracking-wide whitespace-nowrap">
                                {nodeItem.title}
                            </span>
                        )}
                    </span>
                    ...
                    <NextImage
                        className={cn(
                            'h-(--logoloop-logoHeight) w-auto block object-contain grayscale hover:grayscale-0 opacity-60 hover:opacity-100 transition-all',
                        ...
                        src={imageItem.src}
                        width={imageItem.width ?? 100}
                        height={imageItem.height ?? 100}
                        alt={imageItem.alt ?? ''}
